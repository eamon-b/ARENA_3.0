...
class LayerNorm(nn.Module):
    ...
    def forward(self, residual: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
        # just needed to add keepdim=True - careful with broadcasting!
        mean = residual.mean(dim=-1, keepdim=True)
        variance = residual.var(dim=-1, unbiased=False, keepdim=True)
        if self.cfg.debug:
            print(f"{mean.shape=}, {variance.shape=}")
        y = (residual - mean) / (variance + self.cfg.layer_norm_eps).sqrt() * self.w + self.b
        if self.cfg.debug:
            print(f"{y.shape=}")
        return y
...
class Embed(nn.Module):
    ...
    def forward(self, tokens: Int[Tensor, "batch position"]) -> Float[Tensor, "batch position d_model"]:
        # my first way was unnecesarily complicated, just index!
        # return F.one_hot(tokens, num_classes=cfg.d_vocab).float() @ self.W_E
...
class PosEmbed(nn.Module):
    ...
    def forward(self, tokens: Int[Tensor, "batch position"]) -> Float[Tensor, "batch position d_model"]:
        ...
        # return t.stack([self.W_pos[i] for i in range(sequence_length)], dim=0)
        # or using einops:
        return einops.repeat(self.W_pos[:sequence_length], "seq d_model -> batch seq d_model", batch=batch)
...
class Attention(nn.Module):
    ...
    def forward(self, normalized_resid_pre: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
        ...
        if self.cfg.debug:
            print(f"{normalized_resid_pre.shape=}")
        repeated_pre = einops.repeat(
            normalized_resid_pre,
            "b posn d_model -> b n_heads posn d_model",
            n_heads=self.cfg.n_heads
        )
        keys = repeated_pre @ self.W_K
        if self.cfg.debug:
            assert (
                keys.shape == (batch, self.cfg.n_heads, seq_len, self.cfg.d_head)), (
                    f"{keys.shape=},"
                    " expected {(batch, self.cfg.n_heads, seq_len, self.cfg.d_head)}"
                )
            print(f"{keys.shape=}")
        keys += einops.repeat(self.b_K, "n_heads d_head -> n_heads seq_len d_head", seq_len=seq_len)

        queries = repeated_pre @ self.W_Q
        queries += einops.repeat(self.b_Q, "n_heads d_head -> n_heads seq_len d_head", seq_len=seq_len)

        attn_scores = queries @ einops.rearrange(keys, "b n_heads seq_len d_head -> b n_heads d_head seq_len")
        attn_scores =  attn_scores / math.sqrt(self.cfg.d_head)
        attn_scores = self.apply_causal_mask(attn_scores)
        attn_probs = attn_scores.softmax(dim=-1)
        if self.cfg.debug:
            assert attn_probs.shape == (batch, self.cfg.n_heads, seq_len, seq_len)
            print(f"{attn_probs.shape=}")

        values = repeated_pre @ self.W_V
        values += einops.repeat(self.b_V, "n_heads d_head -> n_heads seq_len d_head", seq_len=seq_len)
        if self.cfg.debug:
            assert values.shape == (batch, self.cfg.n_heads, seq_len, self.cfg.d_head)
            print(f"{values.shape=}")

        attended_values = attn_probs @ values

        residual = einops.einsum(attended_values, self.W_O, 'i j k l, j l m -> i j k m')
        # I realised at the end that einsum would be good for this...
        residual = residual.sum(dim=1) # NOTE: sum over heads then add bias
        if self.cfg.debug:
    ...
    # quite a different set up, but einsum is quite good for this when dims are not
    # quite already in the right spot, no need to rearrange, transpose etc
    def forward_solution(self, normalized_resid_pre: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, resid_pre: Float[Tensor, "batch position d_model"]) -> Float[Tensor, "batch position d_model"]:
        x1 = resid_pre + self.attn(self.ln1(resid_pre))
...
class TransformerTrainer:
    ...
    def training_step(self, batch: dict[str, Int[Tensor, "batch seq"]]) -> Float[Tensor, ""]:
        ...
        # NOTE: the model outputs its predictions after each token. very efficient
        logits = self.model(tokens)
        # this function implicitly deals with splitting src using tokens as target
    ...
    def evaluate(self) -> float:
        ...
        for batch in self.test_loader:
            tokens = batch["tokens"]
            logits = self.model(tokens)
            correct = logits[:,:-1].argmax(dim=-1) == tokens[:, 1:]
...
class TransformerSampler:
    ...
    def sample(self, prompt: str, max_tokens_generated=100, verbose=False, **kwargs) -> str:
        ...
        for _ in range(max_tokens_generated):
            ...
            next_token = self.sample_next_token(input_ids.squeeze(0), logits, **kwargs)
            input_ids = torch.concat([input_ids, torch.tensor([next_token], device=device)], dim=-1)
            if verbose:
    ...
    def apply_frequency_penalty(
        ...
        occurances = input_ids.bincount(minlength=d_vocab)
        return logits - occurances * freq_penalty
    ...
    def sample_basic(logits: Float[Tensor, "d_vocab"]) -> int:
        ...
        return distribution.sample((1,)).item()
    ...
    def sample_top_k(logits: Float[Tensor, "d_vocab"], k: int) -> int:
        ...
        """
        top_k_logits, top_k_ids = torch.topk(logits, k)
        sample_idx = torch.distributions.categorical.Categorical(logits=top_k_logits).sample()
        return top_k_ids[sample_idx].item()
    ...
    def sample_top_p(logits: Float[Tensor, "d_vocab"], top_p: float, min_tokens_to_keep: int = 1) -> int:
        ...
        number = torch.searchsorted(cumulative_probs, top_p, right=True)
        sample_idx = torch.distributions.categorical.Categorical(logits=sorted_logits[:number+1]).sample()
        return sorted_ids[sample_idx].item()
...
class Beams:
    ...
    def generate(self, k: int, no_repeat_ngram_size: int | None = None) -> "Beams":
        ...
        log_probs: Tensor = logits.log_softmax(dim=-1)
        top_logprobs, top_tokens = self.get_topk_non_repeating(log_probs, no_repeat_ngram_size, k)
        new_logprob_sums = einops.repeat(self.logprob_sums, "b -> b k", k=k) + top_logprobs
        new_tokens = einops.repeat(self.tokens, "b l -> b k l", k=k)
        new_tokens = torch.concat([new_tokens, top_tokens.unsqueeze(-1)], dim=-1)
    ...
    def filter(self, k: int) -> tuple["Beams", "Beams"]:
        ...
        """
        # first find all that have terminated
        terminated: Tensor = self.tokens[:, -1] == self.tokenizer.eos_token_id
        tokens_terminated = self.tokens[terminated]
        logprob_sums_terminated = self.logprob_sums[terminated]
        if len(logprob_sums_terminated) > k:
            topk_terminated_logprobs, topk_terminated_idx = logprob_sums_terminated.topk(k)
            early_terminations = Beams(self.model, self.tokenizer, topk_terminated_logprobs, tokens_terminated[topk_terminated_idx])
        else:
            early_terminations = Beams(self.model, self.tokenizer, logprob_sums_terminated, tokens_terminated)

        tokens_not_terminated = self.tokens[~terminated]
        logprob_sums_not_terminated = self.logprob_sums[~terminated]
        if len(tokens_not_terminated) > k:
            topk_not_terminated_logprobs, topk_not_terminated_idx = logprob_sums_not_terminated.topk(k)
            best_beams = Beams(self.model, self.tokenizer, topk_not_terminated_logprobs, tokens_not_terminated[topk_not_terminated_idx])
        else:
            best_beams = Beams(self.model, self.tokenizer, logprob_sums_not_terminated, tokens_not_terminated)
