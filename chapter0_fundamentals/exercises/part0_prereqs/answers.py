...
def rearrange_1() -> Tensor:
    ...
    """
    values = t.arange(3, 9)
    return einops.rearrange(values, "(r c) -> r c", c=2)
...
def temperatures_differences(temps: Tensor) -> Tensor:
    ...
    """
    assert len(temps) % 7 == 0
    week_mean = temperatures_average(temps)
    temps = einops.rearrange(temps, "(t2 t1) -> t1 t2", t1=7)
    diffs = temps - week_mean
    return einops.rearrange(diffs, "t1 t2 -> (t2 t1)")
...
def temperatures_normalized(temps: Tensor) -> Tensor:
    ...
    """
    av = einops.reduce(temps, "(t1 7) -> t1", "mean")
    std = einops.reduce(temps, "(t1 7) -> t1", t.std)
    diff = temps - einops.repeat(av, "t -> (t 7)")
    return diff / einops.repeat(std, "t -> (t 7)")
...
def normalize_rows(matrix: Tensor) -> Tensor:
    ...
    """
    l2_norm = matrix.norm(dim=1, keepdim=True)
...
def cos_sim_matrix(matrix: Tensor) -> Tensor:
    ...
    matrix: shape (m, n)
    """
    return normalize_rows(matrix) @ normalize_rows(matrix).T
...
def gather_2d(matrix: Tensor, indexes: Tensor) -> Tensor:
    ...
    assert all(
        [
            ...
            for i, j in itertools.product(*map(range, indexes.shape))
...
def batched_logsumexp(matrix: Tensor) -> Tensor:
    ...
    """
    # next time split into a few steps. Good thinking using einops to start
    return matrix.max(dim=1).values + (matrix - matrix.max(dim=-1, keepdim=True).values).exp().sum(dim=-1).log()
...
def batched_softmax(matrix: Tensor) -> Tensor:
    ...
    """
    return matrix.exp() / matrix.exp().sum(dim=-1, keepdim=True)
...
def batched_logsoftmax(matrix: Tensor) -> Tensor:
    ...
    subtract_max = matrix - row_max
    logsumexp = row_max + subtract_max.exp().sum(dim=-1, keepdim=True).log()
...
def batched_cross_entropy_loss(logits: Tensor, true_labels: Tensor) -> Tensor:
    ...
    indices = einops.rearrange(true_labels, "b -> b 1")
    prediction_at_true = log_prob.gather(1, indices)
    return -einops.rearrange(prediction_at_true, "n 1 -> n")
...
def collect_rows(matrix: Tensor, row_indexes: Tensor) -> Tensor:
    ...
    """
    # first way...
    # row_indices = einops.repeat(row_indexes, "k -> k repeat", repeat=matrix.shape[1])
    # return matrix.gather(0, row_indices)
    # it's much easier!
    assert column_indexes.max() < matrix.shape[0]
