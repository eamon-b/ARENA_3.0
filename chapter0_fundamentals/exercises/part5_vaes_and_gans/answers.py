...
class AutoencoderTrainer:
    ...
    def train(self) -> Autoencoder:
        ...
        for epoch in range(self.args.epochs):
            pbar = tqdm(self.trainloader, desc=f"Training loop epoch {epoch}")
...
class VAE(nn.Module):
    ...
    def __init__(self, latent_dim_size: int, hidden_dim_size: int):
        ...
        self.encoder = Sequential(
            ...
            Rearrange("b (a c) -> a b c", a=2, c=self.latent_dim_size)
        )
        self.decoder = Sequential(
            Linear(in_features=self.latent_dim_size, out_features=self.hidden_dim_size),
            ReLU(),
    ...
    def sample_latent_vector(self, x: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        ...
        epsilon = t.randn_like(log_std)
        return mean + log_std.exp() * epsilon, mean, log_std
    ...
    def forward(self, x: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        ...
        """
        latent, mean, log_std = self.sample_latent_vector(x)
        return self.decoder(latent), mean, log_std
...
class VAETrainer:
    ...
    def training_step(self, img: Tensor):
        ...
        Performs a training step on the batch of images in `img`. Returns the loss. Logs to wandb if enabled.
        """
        reconstructed, mean, log_sigma = self.model.forward(img)
        KL_loss = 0.5 * (log_sigma.exp().pow(2) + mean.pow(2) - 1) - log_sigma
        KL_loss = self.args.beta_kl * KL_loss.mean()
        reconstruction_loss = nn.MSELoss()(img, reconstructed)
    ...
    def __init__(
        ...
        self.project_and_reshape = Sequential(
            Linear(
                ...
                out_features=(img_size // 2**n_layers)**2 * hidden_channels[-1],
                bias=False
            ),
            Rearrange(
        ...
        num_channels = list(reversed(hidden_channels))
        num_channels.append(img_channels)
        self.hidden_layers = Sequential(
            *[
                Sequential(
                    ConvTranspose2d(in_channels=num_channels[i], out_channels=num_channels[i+1], kernel_size=4, stride=2, padding=1),
                    nn.Identity() if i == n_layers - 1 else BatchNorm2d(num_features=num_channels[i+1]),
...
class Discriminator(nn.Module):
    def __init__(
        ...
        super().__init__()
        num_channels = [img_channels] + hidden_channels

        self.hidden_layers = Sequential(
            *[
                Sequential(
                    Conv2d(in_channels=num_channels[i], out_channels=num_channels[i+1], kernel_size=4, stride=2, padding=1),
                    nn.Identity() if i == 0 else BatchNorm2d(num_features=num_channels[i+1]),
                    LeakyReLU()
                )
        ...
        self.classifier = Sequential(
            Rearrange("b c h w -> b (c h w)"),
            Linear(in_features=hidden_channels[-1] * (img_size // 2**3) * (img_size // 2**3), out_features=1, bias=False),
            Sigmoid()
        )
...
def initialize_weights(model: nn.Module) -> None:
    ...
    for module in model.modules():
        if isinstance(module, BatchNorm2d):
            # initialise according to N(1, 0.02)
            nn.init.normal_(module.weight, 1, 0.02)
            nn.init.constant_(module.bias, 0)
        elif isinstance(module, (ConvTranspose2d, Conv2d, Linear)):
            # initialise according to N(0, 0.02)
...
class DCGANTrainer:
    ...
    def training_step_discriminator(
        ...
        """
        self.optD.zero_grad()
        fake_discrim = self.model.netD(img_fake)
        real_discrim = self.model.netD(img_real)
        loss: Tensor = (1 - fake_discrim).log() + real_discrim.log()
        loss *= -1 # maximise
        loss.clamp_(min=-100)
        loss = loss.mean()
        loss.backward()
        ...
        if self.args.use_wandb:
            wandb.log({"discim_loss": loss}, step=self.step)
        return loss
    ...
    def training_step_generator(self, img_fake: Float[Tensor, "batch channels height width"]) -> Float[Tensor, ""]:
        ...
        """
        self.optG.zero_grad()
        discrim: Tensor = self.model.netD(img_fake)
        loss = - discrim.log()
        loss.clamp_(min=-100)
        loss = loss.mean()
        loss.backward()
        ...
        self.optG.step()
        if self.args.use_wandb:
            wandb.log({"gen_loss": loss}, step=self.step)
        return loss
    ...
    def train(self) -> DCGAN:
        ...
        for epoch in range(self.args.epochs):
            ...
            for img_real, label in progress_bar:
                ...
                z = torch.randn((batch_size, self.args.latent_dim_size)).to(device)
                img_fake = self.model.netG(z)
                gen_loss = self.training_step_generator(img_fake)
                discrim_loss = self.training_step_discriminator(img_real, img_fake.detach())
                progress_bar.set_postfix(epoch=epoch, gen_loss=gen_loss.item(), discrim_loss=discrim_loss.item(), step=self.step)
                self.step += 1
