...
class Linear(nn.Module):
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        self.weight = nn.Parameter(
            (torch.rand((out_features, in_features)) - 0.5) * 2 / torch.sqrt(torch.tensor(in_features))
        )
        if bias:
            self.bias = nn.Parameter(
                (torch.rand(out_features) - 0.5) * 2 / torch.sqrt(torch.tensor(in_features))
            )
        else:
            self.bias = None
    ...
    def extra_repr(self) -> str:
        return f"{self.infeatures=}, {self.out_features=}, bias={self.bias is not None}"
...
def train(args: SimpleMLPTrainingArgs) -> tuple[list[float], list[float], SimpleMLP]:
    ...
    return loss_list, accuracy_list, model.cpu()
...
class Conv2d(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int, stride: int = 1, padding: int = 0):
        ...
        self.weight = nn.Parameter(
            (2 * torch.rand(out_channels, in_channels, kernel_size, kernel_size) - 1)
            / torch.sqrt(torch.tensor(self.kernel_size * self.kernel_size * self.in_channels))
        )
...
class Sequential(nn.Module):
    ...
    def __init__(self, *modules: nn.Module):
        ...
        # handle case where *modules is an OrderedDict
...
class BatchNorm2d(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        reshape = lambda x: einops.rearrange(x, "n -> 1 n 1 1")
        ...
        self.model = Sequential(
            ...
            *[
                BlockGroup(n_blocks, in_feats, out_feats, first_stride)
                    for n_blocks, in_feats, out_feats, first_stride in zip(self.n_blocks_per_group, [64] + self.out_features_per_group[:-1], self.out_features_per_group, self.first_strides_per_group)
            ],
            AveragePool(),
