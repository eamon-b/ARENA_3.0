...
def opt_fn_with_sgd(
    ...
    xy_list = [xy.detach().clone()] # don't forget to clone this one too!
    ...
    return torch.stack(xy_list).clone()
...
class SGD:
    ...
    def step(self) -> None:
        ...
        assert all(param.requires_grad for param in self.params)
        grads = [param.grad for param in self.params]
        if self.lmda != 0:
            grads = [grad + self.lmda * theta for grad, theta in zip(grads, self.params)]

        if self.mu != 0:
            for b, grad in zip(self.b, grads):
                # these do actually update the list in place!
                # += is different to x = x + y
                b *= self.mu
                b += grad
            # self.b = [self.mu * b + grad for b, grad in zip(self.b, grads)]
            grads = self.b
    ...
    # better way from the solution, doing it for each param separately:
    @t.inference_mode()
    def step(self) -> None:
        """Performs a single optimization step of the SGD algorithm."""
...
class RMSprop:
    ...
    def step(self) -> None:
        for theta, b, v in zip(self.params, self.b, self.v):
            ...
            if self.mu > 0:
                b.copy_(self.mu * b + g / (v.sqrt() + self.eps))
                theta -= self.lr * b
            else:
                theta -= self.lr * g / (v.sqrt() + self.eps)
    ...
    def step(self) -> None:
        for theta, m, v in zip(self.params, self.m, self.v):
            ...
            if self.lmda != 0:
                g = g + self.lmda * theta
            m.mul_(self.beta1)
            m.add_((1 - self.beta1) * g)
            v.mul_(self.beta2)
            v.add_((1 - self.beta2) * g.pow(2))
...
plot_fn_with_points(rosenbrocks_banana_func, x_range=(-2.5, 2.5), y_range=(-2, 4), z_range=(0, 100), min_points=[(1, 1)], points=points)
...
def update_args(args: WandbResNetFinetuningArgs, sampled_parameters: dict) -> WandbResNetFinetuningArgs:
    ...
    if sampled_parameters["use_weight_decay"] == 1:
