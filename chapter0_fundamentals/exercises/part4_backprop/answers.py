...
def unbroadcast(broadcasted: Arr, original: Arr) -> Arr:
    ...
    """
    # first sum over the prepended dimensions
    num_prepended = broadcasted.ndim - original.ndim
    broadcasted = broadcasted.sum(axis=(tuple(range(num_prepended))))
    # now sum over dims where orginal has size 1
...
def multiply_back0(grad_out: Arr, out: Arr, x: Arr, y: Arr | float) -> Arr:
    ...
    grad_x_broadcasted = grad_out * y
    return unbroadcast(grad_x_broadcasted, x)
...
def multiply_back1(grad_out: Arr, out: Arr, x: Arr | float, y: Arr) -> Arr:
    ...
    if not isinstance(x, Arr):
        x = np.array(x)
    # first find derivate wrt broadcasted, then unbroadcast
    broadcasted_y_grad = grad_out * x
    return unbroadcast(broadcasted_y_grad, y)
...
def forward_and_back(a: Arr, b: Arr, c: Arr) -> tuple[Arr, Arr, Arr]:
    ...
    d_f = log_back(final_grad_out, g, f)
    d_d = multiply_back0(d_f, f, d, e)
    d_e = multiply_back1(d_f, f, d, e)
    d_c = log_back(d_e, e, c)
    d_a = multiply_back0(d_d, d, a, b)
    d_b = multiply_back1(d_d, d, a, b)
...
class BackwardFuncLookup:
    ...
    # map forward function and argument index to its backward function
    back_funcs: dict[tuple[Callable, int], Callable] = {}
...
def log_forward(x: Tensor) -> Tensor:
    out_tensor = Tensor(np.log(x.array), requires_grad=grad_tracking_enabled and x.requires_grad)
    if out_tensor.requires_grad:
        out_tensor.recipe = Recipe(np.log, args=(x.array,), kwargs={}, parents={0:x})
    return out_tensor
...
def multiply_forward(a: Tensor | int, b: Tensor | int) -> Tensor:
    ...
    out_array = np.multiply(arg_a, arg_b)
    requires_grad = grad_tracking_enabled and any(x.requires_grad for x in [a, b] if isinstance(x, Tensor))
    out_tensor = Tensor(out_array, requires_grad=requires_grad)
    if out_tensor.requires_grad:
        parents = {
            index: tensor
            for index, tensor in enumerate([a, b]) if isinstance(tensor, Tensor)
        }
        out_tensor.recipe = Recipe(np.multiply, args=(arg_a, arg_b), kwargs={}, parents=parents)
    return out_tensor
...
def wrap_forward_fn(numpy_func: Callable, is_differentiable=True) -> Callable:
    ...
    def tensor_func(*args: Any, **kwargs: Any) -> Tensor:
        ...
        requires_grad = (
            ...
            and any(x.requires_grad for x in args if isinstance(x, Tensor))
        )
        out = Tensor(out_array, requires_grad=requires_grad)
        if requires_grad:
            parents = {
                index: tensor
                for index, tensor in enumerate(args) if isinstance(tensor, Tensor)
            }
            out.recipe = Recipe(numpy_func, args=arg_arrays, kwargs=kwargs, parents=parents)
...
def topological_sort(node: Node, get_children: Callable) -> list[Node]:
    ...
    """
    # I was missing these variable _outside_ the recursive function
    ...
    def visit(curr: Node):
        """visit a node and update above variables"""
        if curr in perm: # already added this to result
            return
        if curr in temp:
            raise ValueError("Graph is not acyclic")
        # add children before current node
        for child in get_children(curr):
            visit(child)
            # this is the depth first part, visit all descendents of one child first
        result.append(curr)
        perm.add(curr)
        temp.add(curr)
...
def backprop(end_node: Tensor, end_grad: Tensor | None = None) -> None:
    ...
    for node in sorted_computational_graph(end_node):
        ...
        for idx, parent in node.recipe.parents.items():
            if parent in grads:
                grad_parent = grads[parent]
            else:
                grad_parent = np.zeros_like(parent.array, dtype="float")
                grads[parent] = grad_parent
            backwards_function = BACK_FUNCS.get_back_func(node.recipe.func, idx)
            grad_parent += backwards_function(grad_out, parent.array, *node.recipe.args, **node.recipe.kwargs)
...
def maximum_back0(grad_out: Arr, out: Arr, x: Arr, y: Arr):
    ...
    result =  grad_out * np.where(x > y, 1, 0)
    result = np.where(x == y, 0.5, result)
    return unbroadcast(result, x)


def maximum_back1(grad_out: Arr, out: Arr, x: Arr, y: Arr):
    """Backwards function for max(x, y) wrt y."""
    result = grad_out * np.where(x > y, 0, 1)
    result = np.where(x == y, 0.5, result)
...
class Linear(Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        sf = self.in_features ** -0.5
        self.weight = Parameter(
            Tensor(sf * (2 * np.random.rand(out_features, in_features) - 1))
        )
        if bias:
            self.bias = Parameter(
                Tensor(sf * (2 * np.random.rand(out_features) - 1))
            )
        else:
            self.bias = None
...
def cross_entropy(logits: Tensor, true_labels: Tensor) -> Tensor:
    ...
    """
    # data = logits.array
    # data_minus_row_max = data - data.max(axis=1, keepdims=True)
    # data_sub_exp = np.exp(data_minus_row_max)
    # log_sum_exp = data.max(axis=1) + np.log(data_sub_exp.sum(axis=1))
    # batch_size = logits.shape[0]
    # return -logits[arange(0, batch_size), true_labels] + tensor(log_sum_exp)
    batch_size = logits.shape[0]
